...
pub fn proven<H: Hal>(taps: &TapSet, coeffs_size: usize) -> f32 {
    ...
    let theta = 1.0 - alpha;
    let l_plus = {
        // FIXME magic number 2.0
        let rho_plus = (params.trace_domain_size + 2.0) / params.lde_domain_size;
        let m_plus = 1.0 / (2.0 * (alpha / rho_plus.sqrt() - 1.0));
...
fn parameters<H: Hal>(taps: &TapSet, coeffs_size: usize) -> Params {
    ...
    // FIXME field size is hard-coded to baby bear
...
impl Params {
    ...
    fn e_deep(&self, l_plus: f32) -> f32 {
        // FIXME magic number 2.0
        let h_plus = self.trace_domain_size + 2.0;
